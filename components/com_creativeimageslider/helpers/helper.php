...
class CreativeimagesliderHelper
{
    ...
    private function render_slider($id)
    {
        ...
        // free restrictions
        $cis_inf_scroll_enabled = 0;
        $cis_slider_full_size = 0;
        $cis_touch_enabled = 0;
        $cis_custom_css = '';
        $cis_custom_js = '';
        ...
        foreach($slider_items as $k => $slider_item) {
            ...
            if($slider_item['img_path'] != '') {
                ...
                // 3.2.0
                $imgpath = $slider_item['img_path'];

                $image_array = explode('#joomlaImage', $img_path);
                if(sizeof($image_array) > 0) {
                    $imgpath = $image_array[0];
                }

                $img_parts = explode('/',$imgpath);
