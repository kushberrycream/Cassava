 ...
 */
function CreativeimagesliderBuildRoute( &$query )
{
	static $items;
	...
	if (!$items) {
		$component	= JComponentHelper::getComponent('com_creativeimageslider');
		$menu	= $app->getMenu();
	...
	if (is_array($items))
	{
		...
		if (!$itemid) {
			foreach ($items as $item)
			{
				...
				if (isset($item->query['view']) && $item->query['view'] == 'creativeslider'
		...
		if (!$itemid) {
			foreach ($items as $item)
			{
				if (isset($query['view']) && $query['view'] == 'creativeslider' && isset($item->query['view']) && $item->query['view'] == 'creativeslider')
				{
 ...
 * @return	array
 */
function CreativeimagesliderParseRoute( $segments )
{
	$vars = array();
