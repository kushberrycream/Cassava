 ...
 */
'use strict';import{Helper}from'./helper.min.js?1.5.0';import{Prototypes}from'./prototypes.min.js?1.5.0';export function Panel(element,item){this.set=item.set;this.item=item;this.element=element;this.events=[];this.init=async function(){const createEvents=()=>{['open','opening','closed','closing'].forEach(state=>{this.events[state]=new CustomEvent(`rlta.${state}`,{bubbles:true,detail:this});});};createEvents();};this.init();}
