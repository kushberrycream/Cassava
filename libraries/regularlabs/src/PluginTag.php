...
class PluginTag
{
     ...
     * @return object
     */
    public static function getAttributesFromString(
        $string = '',
        $main_key = 'title',
        $known_boolean_keys = [],
        $key_format = '',
        $keep_escaped_chars = null,
        $convert_numerals = null
    )
    {
        if (empty($string))
        {
        ...
        {
            $key         = StringHelper::toCase($match['key'], $key_format);
    ...
    {
        $elements = ($elements ?? null) ?: array_merge(Html::getBlockElements(), ['span']);
    ...
    {
        $block_elements = ($block_elements ?? null) ?: Html::getBlockElements($excluded_block_elements);
    ...
    {
        $unescaped_chars    = array_keys(self::$protected_characters);
        $keep_escaped_chars = ! is_null($keep_escaped_chars)
            ? ArrayHelper::toArray($keep_escaped_chars)
            : [];

        if ( ! empty($keep_escaped_chars))
        {
            array_walk($unescaped_chars, function (&$char, $key, $keep_escaped_chars) {
                if (is_array($keep_escaped_chars) && ! in_array($char, $keep_escaped_chars, true))
                {
                    return;
                }
     ...
     */
    private static function getAttributeValueFromMatch($match, $known_boolean_keys = [], $keep_escaped_chars = [','], $convert_numerals = true)
    {
        $value = $match['value'];
        ...
        if (
            is_numeric($value)
            && (
                in_array($match['key'], $known_boolean_keys, true)
                || in_array(strtolower($match['key']), $known_boolean_keys, true)
            )
        )
        {
        ...
        if ($convert_numerals && is_numeric($value) && RegEx::match('^[0-9\.]+$', $value))
        {
            $value = $value + 0;
        }
