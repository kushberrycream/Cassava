...
use _JchOptimizeVendor\Illuminate\Support\HtmlString;
use _JchOptimizeVendor\Illuminate\Support\Str;
use _JchOptimizeVendor\Illuminate\Support\Traits\Conditionable;
...
class ComponentAttributeBag implements \ArrayAccess, Htmlable, \IteratorAggregate
{
    ...
    public function __toString()
    {
        ...
        foreach ($this->attributes as $key => $value) {
            ...
            $string .= ' '.$key.'="'.\str_replace('"', '\\"', \trim($value)).'"';
        }
     ...
     * @return static
     */
    public function merge(array $attributeDefaults = [], $escape = \true)
    {
        $attributeDefaults = \array_map(function ($value) use ($escape) {
            return $this->shouldEscapeAttributeValue($escape, $value) ? e($value) : $value;
        }, $attributeDefaults);
        [$appendableAttributes, $nonAppendableAttributes] = collect($this->attributes)->partition(function ($value, $key) use ($attributeDefaults) {
            return 'class' === $key || isset($attributeDefaults[$key]) && $attributeDefaults[$key] instanceof AppendableAttributeValue;
        });
