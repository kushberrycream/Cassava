...
class FastPriorityQueue implements \Iterator, \Countable, \Serializable
{
    ...
    public function insert($value, $priority)
    {
        ...
        if (!isset($this->priorities[$priority])) {
            ...
            $this->maxPriority = null === $this->maxPriority ? $priority : \max($priority, $this->maxPriority);
        }
        ++$this->count;
    }
        ...
        while ($this->valid()) {
            if (\current($this->values[$this->maxPriority]) === $datum) {
                ...
                if (empty($this->values[$this->maxPriority])) {
                    unset($this->values[$this->maxPriority], $this->priorities[$this->maxPriority]);
    ...
    public function current()
    {
        switch ($this->extractFlag) {
            ...
            case self::EXTR_BOTH:
                return ['data' => \current($this->values[$this->maxPriority]), 'priority' => $this->maxPriority];
        }
    }
    ...
    {
        if (\false === \next($this->values[$this->maxPriority])) {
    ...
    protected function nextAndRemove()
    {
        ...
        if (\false === \next($this->values[$this->maxPriority])) {
            unset($this->priorities[$this->maxPriority], $this->values[$this->maxPriority]);
