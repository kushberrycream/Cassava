...
class Paginator implements \Countable, \IteratorAggregate
{
    ...
    /**
     * @param AdapterAggregateInterface|AdapterInterface $adapter
     *
    ...
    /**
     * @param ScrollingStylePluginManager|string $scrollingAdapters
     */
    public static function setScrollingStylePluginManager($scrollingAdapters)
    {
        if (\is_string($scrollingAdapters)) {
            if (!\class_exists($scrollingAdapters)) {
                throw new Exception\InvalidArgumentException(\sprintf('Unable to locate scrolling style plugin manager with class "%s"; class not found', $scrollingAdapters));
            }
            $scrollingAdapters = new $scrollingAdapters(new ServiceManager());
        }
        if (!$scrollingAdapters instanceof ScrollingStylePluginManager) {
            throw new Exception\InvalidArgumentException(\sprintf('Pagination scrolling-style manager must extend ScrollingStylePluginManager; received "%s"', \is_object($scrollingAdapters) ? \get_class($scrollingAdapters) : \gettype($scrollingAdapters)));
        }
    ...
    {
        if (null === static::$scrollingStyles) {
    ...
    {
        return null !== static::$cache && $this->cacheEnabled;
    }
    ...
    protected function _getCacheInternalId()
    {
        ...
        $adapterToSerialize = $adapter instanceof DbSelect ? $adapter->getArrayCopy() : $adapter;
        // @codingStandardsIgnoreEnd
        return \md5(\get_class($adapter).\json_encode($adapterToSerialize).$this->getItemCountPerPage());
    }
    ...
    protected function _createPages($scrollingStyle = null)
    {
        ...
        if (null !== $this->getCurrentItems()) {
            ...
            $pages->firstItemNumber = $pages->totalItemCount ? ($currentPageNumber - 1) * $pages->itemCountPerPage + 1 : 0;
            $pages->lastItemNumber = $pages->totalItemCount ? $pages->firstItemNumber + $pages->currentItemCount - 1 : 0;
        }
