...
use _JchOptimizeVendor\Laminas\Math\BigInteger;
 ...
 * @see       http://pickle-js.googlecode.com
...
class PythonPickle extends AbstractAdapter
{
     ...
     *
     * @see http://hg.python.org/cpython/file/2.7/Lib/pickletools.py
    ...
    // Protocol 2
    public const OP_PROTO = "\x80";
    // identify pickle protocol
    public const OP_NEWOBJ = "\x81";
    // build object by applying cls.__new__ to argtuple
    public const OP_EXT1 = "\x82";
    // push object from extension registry; 1-byte index
    public const OP_EXT2 = "\x83";
    // ditto, but 2-byte index
    public const OP_EXT4 = "\x84";
    // ditto, but 4-byte index
    public const OP_TUPLE1 = "\x85";
    // build 1-tuple from stack top
    public const OP_TUPLE2 = "\x86";
    // build 2-tuple from two topmost stack items
    public const OP_TUPLE3 = "\x87";
    // build 3-tuple from three topmost stack items
    public const OP_NEWTRUE = "\x88";
    // push True
    public const OP_NEWFALSE = "\x89";
    // push False
    public const OP_LONG1 = "\x8a";
    // push long from < 256 bytes
    public const OP_LONG4 = "\x8b";
    ...
    protected static $quoteString = ['\\' => '\\\\', "\x00" => '\\x00', "\x01" => '\\x01', "\x02" => '\\x02', "\x03" => '\\x03', "\x04" => '\\x04', "\x05" => '\\x05', "\x06" => '\\x06', "\x07" => '\\x07', "\x08" => '\\x08', "\t" => '\\t', "\n" => '\\n', "\v" => '\\x0b', "\f" => '\\x0c', "\r" => '\\r', "\x0e" => '\\x0e', "\x0f" => '\\x0f', "\x10" => '\\x10', "\x11" => '\\x11', "\x12" => '\\x12', "\x13" => '\\x13', "\x14" => '\\x14', "\x15" => '\\x15', "\x16" => '\\x16', "\x17" => '\\x17', "\x18" => '\\x18', "\x19" => '\\x19', "\x1a" => '\\x1a', "\x1b" => '\\x1b', "\x1c" => '\\x1c', "\x1d" => '\\x1d', "\x1e" => '\\x1e', "\x1f" => '\\x1f', "\xff" => '\\xff'];
    // process vars
    /** @var int */
    protected $protocol;
    ...
    protected $bigIntegerAdapter;

    /** @var PythonPickleOptions */
    protected $options;

    /**
     * @param array|PythonPickleOptions|\Traversable $options Optional
     */
    public function __construct($options = null)
    {
        // init
        if (null === static::$isLittleEndian) {
            static::$isLittleEndian = "\x01\x00\x00\x00" === \pack('l', 1);
        }
     ...
     * Set options.
     *
     * @param array|PythonPickleOptions|\Traversable $options
     *
     * @return PythonPickle
     */
    ...
    protected function write($value)
    {
        if (null === $value) {
            ...
            throw new Exception\RuntimeException(\sprintf('PHP-Type "%s" can not be serialized by %s', \gettype($value), static::class));
        }
    }
    ...
    protected function writeGet($id)
    {
        if (0 === $this->protocol) {
            ...
            $bin = \pack('l', $id);
            if (\false === static::$isLittleEndian) {
                $bin = \strrev($bin);
            }
    ...
    {
        if ($this->protocol >= 2) {
            $this->pickle .= \true === $value ? self::OP_NEWTRUE : self::OP_NEWFALSE;
        } else {
            $this->pickle .= self::OP_INT.(\true === $value ? '01' : '00')."\r\n";
        }
    }
     ...
     * @param mixed $value
     *
     * @return bool|int The id or false
     */
    protected function searchMemo($value)
    {
        return \array_search($value, $this->memo, \true);
    }
    ...
    protected function hex2Utf8($hex)
    {
        ...
        if ($uniCode < 0x80) {
            ...
            throw new Exception\RuntimeException(\sprintf('Unsupported unicode character found "%s"', \dechex($uniCode)));
        }
    ...
    protected function unquoteString($str)
    {
        ...
        return \strtr(\substr(\trim($str), 1, -1), $quoteArr);
    }
    ...
    {
        for ($k = \count($this->stack) - 1; $k >= 0; --$k) {
    ...
    protected function decodeBinLong($data)
    {
        ...
        if ($nbytes > 7) {
            ...
            $data = \strrev($data);
            $long = $this->bigIntegerAdapter->binToInt($data, \true);
        } else {
            for ($i = 0; $i < $nbytes; ++$i) {
    ...
    protected function setProtocol(int $proto): void
    {
        if ($proto < 2 || $proto > 3) {
            throw new Exception\RuntimeException("Invalid or unknown protocol version '{$proto}' detected");
        }
        $this->protocol = $proto;
    }
}
