...
class Encoder
{
    ...
    public static function encodeClass($className, $package = '')
    {
        ...
        if (!$class->isInstantiable()) {
            throw new InvalidArgumentException(\sprintf('"%s" must be instantiable', $className));
        }

        return \sprintf('Class.create(\'%s%s\',{%s,%s,%s});', $package, $className, self::encodeConstants($class), self::encodeMethods($class), self::encodeVariables($class));
    }
    ...
    protected function encodeObject(&$value)
    {
        if ($this->cycleCheck) {
            if ($this->wasVisited($value)) {
                if (!isset($this->options['silenceCyclicalExceptions']) || \true !== $this->options['silenceCyclicalExceptions']) {
                    throw new RecursionException(\sprintf('Cycles not supported in JSON encoding; cycle introduced by class "%s"', \get_class($value)));
                }
        ...
        if (\method_exists($value, 'toJson')) {
            ...
            foreach ($propCollection as $name => $propValue) {
                ...
                $props .= ','.$this->encodeValue($name).':'.$this->encodeValue($propValue);
            }
        }
        $className = \get_class($value);

        return '{"__className":'.$this->encodeString($className).$props.'}';
    }
    ...
    {
        if (\in_array($value, $this->visited, \true)) {
            return \true;
        }
    ...
    protected function encodeAssociativeArray($array)
    {
        ...
        foreach ($array as $key => $value) {
            $tmpArray[] = \sprintf('%s:%s', $this->encodeString((string) $key), $this->encodeValue($value));
        }
        $result .= \implode(',', $tmpArray);
        $result .= '}';
