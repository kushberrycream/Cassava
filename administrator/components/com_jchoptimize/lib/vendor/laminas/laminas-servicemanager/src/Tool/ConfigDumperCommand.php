...
EOH;
    ...
    public function __invoke(array $args)
    {
        ...
        try {
            $config = $dumper->createDependencyConfig($arguments->config, $arguments->class, $arguments->ignoreUnresolved);
        } catch (Exception\InvalidArgumentException $e) {
            $this->helper->writeErrorMessage(\sprintf('Unable to create config for "%s": %s', $arguments->class, $e->getMessage()));
            $this->help(\STDERR);
    ...
    private function parseArgs(array $args)
    {
        ...
        if (\in_array($arg1, ['-i', '--ignore-unresolved'], \true)) {
            $ignoreUnresolved = \true;
            $arg1 = \array_shift($args);
        }
        if (!$args) {
        ...
        switch (\file_exists($configFile)) {
            case \true:
                ...
                if (!\is_array($config)) {
                    return $this->createErrorArgument(\sprintf('Configuration at path "%s" does not return an array.', $configFile));
                }
            ...
            default:
                if (!\is_writable(\dirname($configFile))) {
                    return $this->createErrorArgument(\sprintf('Cannot create configuration at path "%s"; not writable.', $configFile));
                }
                $config = [];
    ...
    {
        return (object) ['command' => $command, 'configFile' => $configFile, 'config' => $config, 'class' => $class, 'ignoreUnresolved' => $ignoreUnresolved];
    }
    ...
    {
        return (object) ['command' => self::COMMAND_ERROR, 'message' => $message];
    }
    ...
    {
        return (object) ['command' => self::COMMAND_HELP];
    }
}
