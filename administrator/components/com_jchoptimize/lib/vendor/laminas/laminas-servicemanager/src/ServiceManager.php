...
class ServiceManager implements ServiceLocatorInterface
{
    ...
    /**
     * @var Factory\DelegatorFactoryInterface[][]|string[][]
     *
    ...
    /**
     * @var callable[]|Initializer\InitializerInterface[]
     *
    ...
    {
        \trigger_error(\sprintf('Usage of %s is deprecated since v3.0.0; please use the container passed to the factory instead', __METHOD__), \E_USER_DEPRECATED);
     ...
     *
     * @param string                                   $name    Service name
     * @param callable|Factory\FactoryInterface|string $factory Factory to which to map.
     ...
     *
     * @param Factory\AbstractFactoryInterface|string $factory abstract factory
     ...
     *
     * @param string                                            $name    Service name
     * @param callable|Factory\DelegatorFactoryInterface|string $factory delegator
     *                                                                   factory to assign
     *
     ...
     * Add an initializer.
     *
     * @param callable|Initializer\InitializerInterface|string $initializer
     *
    ...
    private function getFactory(string $name): callable
    {
        ...
        throw new ServiceNotFoundException(\sprintf('Unable to resolve service "%s" to a factory; are you certain you provided it during configuration?', $name));
    }
    ...
    private function createDelegatorFromName(string $name, ?array $options = null)
    {
        ...
        foreach ($this->delegators[$name] as $index => $delegatorFactory) {
            ...
            if (LazyServiceFactory::class === $delegatorFactory) {
            ...
            $creationCallback = static fn () => $delegatorFactory($initialCreationContext, $name, $creationCallback, $options);
        }
    ...
    private function doCreate(string $resolvedName, ?array $options = null)
    {
        try {
            ...
            throw new ServiceNotCreatedException(\sprintf('Service with name "%s" could not be created. Reason: %s', $resolvedName, $exception->getMessage()), (int) $exception->getCode(), $exception);
        }
    ...
    private function createLazyServiceDelegatorFactory(): LazyServiceFactory
    {
        ...
        $this->lazyServicesDelegator = new LazyServiceFactory(new LazyLoadingValueHolderFactory($factoryConfig), $this->lazyServices['class_map']);
    ...
    private function mergeDelegators(array $config): array
    {
        foreach ($config as $key => $delegators) {
            ...
            foreach ($delegators as $delegator) {
                if (!\in_array($delegator, $this->delegators[$key], \true)) {
     ...
     *
     * @param Factory\AbstractFactoryInterface|string $abstractFactory
     *
    ...
    private function assertCallableDelegatorFactory($delegatorFactory): void
    {
        ...
        if (\is_string($delegatorFactory)) {
            throw new ServiceNotCreatedException(\sprintf('An invalid delegator factory was registered; resolved to class or function "%s" which does not exist; please provide a valid function name or class name resolving to an implementation of %s', $delegatorFactory, DelegatorFactoryInterface::class));
        }

        throw new ServiceNotCreatedException(\sprintf('A non-callable delegator, "%s", was provided; expected a callable or instance of "%s"', \is_object($delegatorFactory) ? \get_class($delegatorFactory) : \gettype($delegatorFactory), DelegatorFactoryInterface::class));
    }
}
