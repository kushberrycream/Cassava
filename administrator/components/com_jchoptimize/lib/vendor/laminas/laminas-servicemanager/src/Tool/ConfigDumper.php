...
use _JchOptimizeVendor\Laminas\ServiceManager\AbstractFactory\ConfigAbstractFactory;
...
EOC;
    ...
    public function createDependencyConfig(array $config, $className, $ignoreUnresolved = \false)
    {
        ...
        $constructorArguments = \array_filter($constructorArguments, static fn (\ReflectionParameter $argument): bool => !$argument->isOptional());
        ...
        foreach ($constructorArguments as $constructorArgument) {
            ...
            if (null === $argumentType) {
                ...
                throw new InvalidArgumentException(\sprintf('Cannot create config for constructor argument "%s", it has no type hint, or non-class/interface type hint', $constructorArgument->getName()));
            }
    ...
    public function createFactoryMappingsFromConfig(array $config)
    {
        ...
        if (!\is_array($config[ConfigAbstractFactory::class])) {
            throw new InvalidArgumentException('Config key for '.ConfigAbstractFactory::class.' should be an array, '.\gettype($config[ConfigAbstractFactory::class]).' given');
        }
    ...
    {
        $this->validateClassName($className);
        if (\array_key_exists('service_manager', $config) && \array_key_exists('factories', $config['service_manager']) && \array_key_exists($className, $config['service_manager']['factories'])) {
            return $config;
        }
    ...
    public function dumpConfigFile(array $config)
    {
        ...
        return \sprintf(self::CONFIG_TEMPLATE, static::class, \date('Y-m-d H:i:s'), $prepared);
    }
    ...
    private function prepareConfig($config, $indentLevel = 1)
    {
        ...
        foreach ($config as $key => $value) {
            ...
            $entries[] = \sprintf('%s%s%s,', $indent, $key ? \sprintf('%s => ', $key) : '', $this->createConfigValue($value, $indentLevel));
        }
