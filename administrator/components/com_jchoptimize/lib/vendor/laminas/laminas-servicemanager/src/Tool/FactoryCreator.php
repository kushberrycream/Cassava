...
use _JchOptimizeVendor\Laminas\ServiceManager\Factory\FactoryInterface;
...
EOT;
    private const IMPORT_ALWAYS = [FactoryInterface::class, ContainerInterface::class];
    ...
    public function createFactory($className)
    {
        ...
        return \sprintf(self::FACTORY_TEMPLATE, \preg_replace('/\\\\'.$class.'$/', '', $className), $this->createImportStatements($className), $class, $class, $class, $this->createArgumentString($className));
    }
    ...
    private function getConstructorParameters($className)
    {
        ...
        $constructorParameters = \array_filter($constructorParameters, function (\ReflectionParameter $argument): bool {
            ...
            if (null === $class) {
                throw new InvalidArgumentException(\sprintf('Cannot identify type for constructor argument "%s"; no type hint, or non-class/interface type hint', $argument->getName()));
            }
    ...
    {
        $arguments = \array_map(fn (string $dependency): string => \sprintf('$container->get(\\%s::class)', $dependency), $this->getConstructorParameters($className));
