...
{
	if($joomla4) {
		$saveOrderingUrl = 'index.php?option=com_creativeimageslider&task=creativesliders.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
		HTMLHelper::_('draggablelist.draggable');
	} else {
		$saveOrderingUrl = 'index.php?option=com_creativeimageslider&task=creativesliders.saveOrderAjax&tmpl=component';
...
<?php endif;?>
		...
		<table class="table table-striped" id="articleList">
			<thead>
				<tr>
					...
					<th>
						<?php echo JHtml::_('grid.sort', 'COM_CREATIVEIMAGESLIDER_NAME', 'sp.name', $listDirn, $listOrder); ?>
					</th>
					<th width="25%">
						Shortcode
					</th>
					...
					<th width="5%">
						<?php echo JHtml::_('grid.sort', 'COM_CREATIVEIMAGESLIDER_NUM_IMAGES', 'num_images', $listDirn, $listOrder); ?>
					</th>
					<th width="1%" class="nowrap center hidden-phone">
			...
			foreach ($this->items as $i => $item) :
				...
				<tr class="row<?php echo $i % 2; ?>" sortable-group-id="7" data-draggable-group="7">
					...
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->published, $i, 'creativesliders.', true, 'cb', $item->publish_up, $item->publish_down); ?>
					</td>
					<td class="nowrap has-context">
						<div class="pull-left">
							<a href="<?php echo JRoute::_('index.php?option=com_creativeimageslider&task=creativeslider.edit&id='.(int) $item->id); ?>">
								<?php echo $this->escape($item->name); ?>
							</a>
						</div>
					</td>
					<td>
						<input class="creative_shortcode" value='[creativeimageslider id=&quot;<?php echo $item->id;?>&quot;]' onclick="this.select()" readonly="readonly" />
					</td>
					<td align="small hidden-phone">
						<a href="<?php echo JRoute::_('index.php?option=com_creativeimageslider&task=creativecategory.edit&id='.(int) $item->category_id); ?>">
							<?php echo $item->category_title; ?>
						</a>
					</td>
					<td align="center hidden-phone">
						<?php echo $item->num_images; ?>
					</td>
					<td align="center hidden-phone">
						<?php echo $item->id; ?>
					</td>
		...
		<input type="hidden" name="view" value="creativesliders" />
