...
class CreativeimagesliderModelCreativeTemplate extends JModelAdmin
{
	/**
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'CreativeTemplate', $prefix = 'CreativeTemplateTable', $config = array()) 
	{
	...
	public function getStyles() {
		if(isset($_GET['id']))
			$id = (int)$_GET['id'];
		$db		= $this->getDbo();
		$sql = "SELECT `styles` FROM `#__cis_templates` WHERE `id` = ".$id;
		$db->setQuery($sql);
	...
	{
		// Get the form.
		$form = $this->loadForm('com_creativeimageslider.creativetemplate', 'creativetemplate', array('control' => '', 'load_data' => $loadData));
		if (empty($form)) 
		{
			return false;
		}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_creativeimageslider.edit.creativetemplate.data', array());
		if (empty($data)) 
		{
			$data = $this->getItem();
		}
	...
	public function store()
	{
		...
		if($id == 0) {
			...
			$query = ' SELECT * FROM #__cis_templates '.
			...
			if (!$this->_db->insertObject( '#__cis_templates', $new_template, 'id' ))
				return false;
		}
			...
			if (!$this->_db->updateObject( '#__cis_templates', $new_template, 'id' ))
				return false;
		}
		return true;
	}
