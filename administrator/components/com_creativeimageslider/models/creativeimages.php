...
class CreativeimagesliderModelCreativeimages extends JModelList {
	...
	// 4.6.3
	public function getCreativeMinSliderId() {
		$db		= $this->getDbo();
		$query = "SELECT min(`id`) FROM #__cis_images";
		$db->setQuery($query);
		$min_slider_id = $db->loadResult();

		return $min_slider_id;
	}
	...
	protected function getListQuery()
	{
		...
		// 4.6.3
		$min_slider_id = $this->getCreativeMinSliderId();
		...
		// Filter by  sliders
		// 4.6.3
		$sliderId = (int)$this->getState('filter.slider_id');
		$sliderId = $sliderId == 0 ? $min_slider_id : $sliderId;
